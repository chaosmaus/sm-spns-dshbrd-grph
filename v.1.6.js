        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                function formatPercentage(value) {
                    ...
                    if (number % 1 !== 0) {
                        return number.toFixed(1) + '%'; // If there's a decimal, format to 1 decimal place
                    } else {
                        return number.toFixed(0) + '%'; // If no decimal, show as an integer
                    }
                }
                
                $('#annual_premium_increase').text(formatPercentage(response.responseData.model.annual_premium_percentage_yoy_growth_rate).toFixed(0));
                $('#cost_of_reinsurance').text(formatPercentage(response.responseData.model.reinsurance_percentage_of_premium).toFixed(0));
    ...
    const initiateGraph = async (data) => {
        ...
        data.forEach((element, index) => {
            ...
            values.forEach((el, i) => {
                ...
                $('.dashboard--graph--column').eq(index).find('.dashboard--graph--point--text').eq(i).text('$' + formatValue(elValue).toFixed(0));
