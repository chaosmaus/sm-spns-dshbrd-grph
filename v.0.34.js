        ...
        $('.dashboard--graph--column').each((i, el) => {
            ...
            blocks.each((index, element) => {
                blocksArray.push({
                    ...
                    value: parseFloat($(element).css('height')) // Convert height to a numeric value
                });
            });
        });
    
        // Sort blocksArray by height in ascending order
        blocksArray.sort((a, b) => a.value - b.value);
    
        // Assign z-index values based on the sorted order
        blocksArray.forEach((block, index) => {
            $(`#${block.name}`).css('z-index', index + 1);
        });
    };
