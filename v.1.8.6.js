...
$(async function () {
    ...
    const getModel = async (estimatedClaims = null, annualPremium = null) => {
        ...
        const auth0Client = await auth0.createAuth0Client({
            ...
            authorizationParams: {
                audience: "https://server.xncaptive.com"
            },
        });
        const token = await auth0Client.getTokenSilently();
        $.ajax({
            url: 'https://server.xncaptive.com/api/users/update-model',
            type: 'POST',
            headers: {
            ...
            success: function (response) {
                ...
                $('.dhome--assumptions-form').removeClass('active')
    ...
    const graphPopupHandler = () => {
        $('.graph--popup--mask').css('display', 'flex')
        $('.graph--popup--close--button').on('click', ()=>{
            $('.graph--popup--mask').hide()
        })
    }
    ...
    const initiateGraph = async (data) => {
        ...
        data.forEach((element, index) => {
            ...
            // Show a message if cumulative_surplus is 0
            if(cumulative_surplus === 0 ) graphPopupHandler();
