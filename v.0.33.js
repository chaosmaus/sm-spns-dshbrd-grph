...
$(async function () {
    ...
    const initiateGraph = async (data) => {
        ...
        function setZIndexBasedOnValue(values, index) {
            // Create a shallow copy of the array and sort the copy by the value property, in ascending order
            const sortedValues = [...values].sort((a, b) => a.value - b.value);
        
            // Loop through the sorted array
            sortedValues.forEach((item, i) => {
                // Set the z-index of each element based on its position in the sorted array
                // Use the name as a class selector to target the correct elements
                // It seems there was a typo in setting z-index to 'index', which doesn't correspond to the sorted position
                // Instead, set z-index based on 'i' to reflect the element's sorted position
                $('.dashboard--graph--column').find(`.${item.name}`).css('z-index', i + 1); // Adjusted to 'i + 1' to ensure z-index starts from 1
            });
        }
